...
class CommandOption {
  ...
  CommandOption._new(Map<String, dynamic> raw) {
    ...
    this.choices = [
      if (raw["choices"] != null)
        for (final choiceRaw in raw["choices"]) ArgChoice._new(choiceRaw as Map<String, dynamic>)
    ];

    this.options = [
      if (raw["options"] != null)
        for (final optionRaw in raw["options"]) CommandOption._new(optionRaw as Map<String, dynamic>)
    ];
  }
}
