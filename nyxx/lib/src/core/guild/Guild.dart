...
class Guild extends SnowflakeEntity {
  ...
  Iterable<GuildChannel> get channels =>
      this.client.channels.find((item) => item is GuildChannel && item.guild.id == this.id).cast();
  ...
  Guild._new(this.client, Map<String, dynamic> raw, [bool guildCreate = false]) : super(Snowflake(raw["id"])) {
    ...
    if (raw["public_updates_channel_id"] != null) {
      this.publicUpdatesChannel =
          CacheableTextChannel<TextChannel>._new(client, Snowflake(raw["public_updates_channel_id"]));
    }
  }
  ...
  String guildWidgetUrl([String style = "shield"]) => client._httpEndpoints.getGuildWidgetUrl(this.id, style);

  /// Fetches emoji from API
  Future<IGuildEmoji> fetchEmoji(Snowflake emojiId) => client._httpEndpoints.fetchGuildEmoji(this.id, emojiId);
  ...
  Future<GuildEmoji> createEmoji(String name,
      ...
      client._httpEndpoints.createEmoji(this.id, name,
          roles: roles,
          imageFile: imageFile,
  ...
  Future<void> changeSelfNick(String nick) async => client._httpEndpoints.changeGuildSelfNick(this.id, nick);

  /// Gets single [Ban] object for given [bannedUserId]
  Future<Ban> getBan(Snowflake bannedUserId) async => client._httpEndpoints.getGuildBan(this.id, bannedUserId);
  ...
  /// Leaves the guild.
  Future<void> leave() async => client._httpEndpoints.leaveGuild(this.id);

  /// Returns list of Guilds invites
  Stream<Invite> fetchGuildInvites() => client._httpEndpoints.fetchGuildInvites(this.id);
  ...
  Future<AuditLog> fetchAuditLogs({Snowflake? userId, int? actionType, Snowflake? before, int? limit}) =>
      client._httpEndpoints
          .fetchAuditLogs(this.id, userId: userId, actionType: actionType, before: before, limit: limit);
  ...
  Stream<VoiceRegion> getVoiceRegions() => client._httpEndpoints.fetchGuildVoiceRegions(this.id);
  ...
  /// Unbans a user by ID.
  Future<void> unban(Snowflake id, Snowflake userId) => client._httpEndpoints.guildUnban(this.id, userId);
  ...
  Future<Member> fetchMember(Snowflake memberId) => client._httpEndpoints.fetchGuildMember(this.id, memberId);
  ...
  Stream<Member> searchMembersGateway(String query, {int limit = 0}) async* {
    ...
    if (first.chunkCount > 1) {
      await for (final event
          in this.shard.onMemberChunk.where((event) => event.nonce == nonce).take(first.chunkCount - 1)) {
  ...
  Future<GuildPreview> fetchGuildPreview() async => this.client.httpEndpoints.fetchGuildPreview(this.id);
  ...
  /// Deletes the guild.
  Future<void> delete() => client._httpEndpoints.deleteGuild(this.id);
}
