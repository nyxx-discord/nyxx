...
Future<void> _shardHandler(SendPort shardPort) async {
  ...
  Future<void> _connect() async {
    await WebSocket.connect(gatewayUri.toString()).then((ws) {
      final zlibDecoder = RawZLibFilter
          .inflateFilter(); // Create zlib decoder specific to this connection. New connection should get new zlib context
      ...
      _socketSubscription = _socket!.listen((data) {
        ...
        shardPort.send({"cmd": "DISCONNECTED", "errorCode": _socket!.closeCode, "errorReason": _socket!.closeReason});
          ...
          cancelOnError: true,
          onError: (err) => shardPort.send({
                "cmd": "ERROR",
                "error": err.toString(),
                "errorCode": _socket!.closeCode,
                "errorReason": _socket!.closeReason
              }));

      shardPort.send({"cmd": "CONNECT_ACK"});
    },
        onError: (err, __) => shardPort.send({
              "cmd": "ERROR",
              "error": err.toString(),
              "errorCode": _socket!.closeCode,
              "errorReason": _socket!.closeReason
