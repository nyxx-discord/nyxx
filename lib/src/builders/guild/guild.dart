...
class GuildUpdateBuilder extends UpdateBuilder<Guild> {
  ...
  Map<String, Object?> build() => {
        ...
        if (!identical(splash, sentinelImageBuilder)) 'splash': splash?.build(),
        if (!identical(discoverySplash, sentinelImageBuilder)) 'discoverySplash': discoverySplash?.build(),
        if (!identical(banner, sentinelImageBuilder)) 'banner': banner?.build(),
