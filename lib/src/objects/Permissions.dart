...
class Permissions {
  ...
  Permissions.fromInt(int permissions) {
    ...
    this.createInstantInvite =
        (this.raw & Constants.permissions['CREATE_INSTANT_INVITE']) > 0;
    ...
    this.administrator =
        (this.raw & Constants.permissions['ADMINISTRATOR']) > 0;
    this.manageChannels =
        (this.raw & Constants.permissions['MANAGE_CHANNELS']) > 0;
    ...
    this.sendTtsMessages =
        (this.raw & Constants.permissions['SEND_TTS_MESSAGES']) > 0;
    this.manageMessages =
        (this.raw & Constants.permissions['MANAGE_MESSAGES']) > 0;
    ...
    this.readMessageHistory =
        (this.raw & Constants.permissions['READ_MESSAGE_HISTORY']) > 0;
    this.mentionEveryone =
        (this.raw & Constants.permissions['MENTION_EVERYONE']) > 0;
    this.useExternalEmojis =
        (this.raw & Constants.permissions['EXTERNAL_EMOJIS']) > 0;
    ...
    this.deafenMembers =
        (this.raw & Constants.permissions['DEAFEN_MEMBERS']) > 0;
    this.moveMembers = (this.raw & Constants.permissions['MOVE_MEMBERS']) > 0;
    this.useVad = (this.raw & Constants.permissions['USE_VAD']) > 0;
    this.changeNickname =
        (this.raw & Constants.permissions['CHANGE_NICKNAME']) > 0;
    this.manageNicknames =
        (this.raw & Constants.permissions['MANAGE_NICKNAMES']) > 0;
    this.manageRoles =
        (this.raw & Constants.permissions['MANAGE_ROLES_OR_PERMISSIONS']) > 0;
    this.manageWebhooks =
        (this.raw & Constants.permissions['MANAGE_WEBHOOKS']) > 0;
  }
}
