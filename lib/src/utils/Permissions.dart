...
bool canInteract({Member issueMember, Role issueRole, Member targetMember, Role targetRole}) {
  ...
  if (issueRole != null && targetRole != null) {
    if(issueRole.guild != targetRole.guild)
      return false;
...
List<int> getOverrides(Member member, GuildChannel channel) {
  ...
  try {
    var publicOverride = channel.permissions.firstWhere((ov) => ov.id == member.guild.everyoneRole.id);
  ...
  for (var role in member.roles) {
    var chanOveride = channel.permissions.firstWhere((f) => f.id == role.id, orElse: () => null);
  ...
  final memberOverride = channel.permissions.firstWhere((g) => g.id == member.id, orElse: () => null);
...
bool isApplied(int permissions, int permission) => (permissions & permission) == permission;